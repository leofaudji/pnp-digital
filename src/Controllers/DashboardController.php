        ...
        $stats = cache()->remember('dashboard_stats', 300, function () {
            ...
            if ($currentHour >= 7 && $currentHour < 15) {
                ...
                if ($currentHour >= 23) {
                    ...
                    $shiftEnd = (clone $now)->modify('+1 day')->format('Y-m-d') . " 07:00:00";
                } else {
                    $y = (clone $now)->modify('-1 day');
                    $shiftStart = $y->format('Y-m-d') . " 23:00:00";
                    $shiftEnd = $now->format('Y-m-d') . " 07:00:00";
                }
            }
            ...
            $cpMap = [];
            foreach ($stmtCp->fetchAll() as $cp) $cpMap[$cp['id']] = $cp['name'];
            ...
            $stats['current_shift'] = ['name' => $shiftName, 'start' => $shiftStart, 'end' => $shiftEnd];
